...
sol = solve(
    pulse, SimParams(; medium=med, raman_model=BlowWood(), z_saves=2); progress=false
)

# 2. Vectorial medium
med_vec = BirefringentMedium(
    0.1,
    0.001,
    0.0,
    TaylorDispersion([-22.0e-27]),
    TaylorDispersion([-22.0e-27], 1e-12),
    0.0,
    1550e-9,
)
...
edfa = AmplifyingMedium(;
    length=0.1, gamma=0.001, g0_db=5.0, Esat=1e-6, betas=[-22.0e-27], lambda0=1550e-9
)
sol_edfa = solve(
...
sol_hcf = solve(
    pulse, SimParams(; medium=hcf, raman_model=nothing, z_saves=2); progress=false
)

# 5. Semiconductor Waveguide
soi = SemiconductorMedium(;
    length=0.01, gamma=100.0, alpha2=5e-12, Aeff=0.1e-12, betas=[-1000e-27], lambda0=1550e-9
)
sol_soi = solve(
