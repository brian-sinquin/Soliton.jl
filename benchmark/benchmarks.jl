...
params_erk4ip = SimParams(; medium=medium, z_saves=20, raman_model=BlowWood(), self_steepening=true, solver=ERK4IP(rtol=1e-5, atol=1e-7))
params_ssfm = SimParams(; medium=medium, z_saves=20, raman_model=BlowWood(), self_steepening=true, solver=SSFM(5e-5))

SUITE["solvers"]["erk4ip_scalar"] = @benchmarkable solve($pulse, $params_erk4ip; progress=false)
...
SUITE["solvers"]["ssfm_vectorial"] = @benchmarkable solve($vpulse, $params_vssfm; progress=false)
...
SUITE["operators"]["build_physics_model"] = @benchmarkable build_physics_model($grid, $params_erk4ip, $(pulse.At))
SUITE["operators"]["spm_raman_eval"]       = @benchmarkable JuGNLSE._spm_raman($(pulse.AW), $model_scalar, 0.025)
SUITE["operators"]["vectorial_spm_fwm_eval"] = @benchmarkable JuGNLSE._vectorial_spm_fwm($(vpulse.At), $model_vec, 0.025)
