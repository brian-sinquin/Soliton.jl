...
@testset "Vectorial/Birefringent Coupled Solver" begin
    @testset "Type validation" begin
        ...
        @test_throws ArgumentError BirefringentMedium(-0.1, 0.11, 0.0, dis_x, dis_y, 0.0, 835e-9)
        @test_throws ArgumentError BirefringentMedium(0.1, -0.11, 0.0, dis_x, dis_y, 0.0, 835e-9)
    ...
    @testset "Comparison with scalar solver" begin
        ...
        params_scalar = SimParams(; medium=medium_scalar, z_saves=5, solver=SSFM(1e-5), raman_model=nothing)
        ...
        params_vec = SimParams(; medium=medium_vec, z_saves=5, solver=SSFM(1e-5), raman_model=nothing)
    ...
    @testset "Birefringent Walk-off" begin
        ...
        params = SimParams(; medium=medium, z_saves=2, solver=SSFM(5e-5), raman_model=nothing)
